...
trade_us_ctx.start()
trade_us_ctx.unlock_trade("110007")
...
def getMyStocks(ctx):
    ret_code, ret_data = ctx.position_list_query(envtype=envtype)
    if ret_code != 0:
...
while (True):
    ...
    while True:
        ...
        for stockNum in stockNums:
            ...
            ret_code, ret_data = trade_us_ctx.place_order(sellPrice, selfStockData["can_sell_qty"], stockNum,
                                                          orderside=1, ordertype=2, envtype=envtype)  # 下单接口
            if ret_code != 0:
